...
<?php
	...
	function filtrevar($var, $filtre, $complement=""){
		...
		switch($filtre){
			...
			case "string": if(! preg_match("/^[0-9a-zA-Z\._ÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿñ$complement]*$/", $var)) $erreur = 1; break;
			case "float" : if(! preg_match("/^[0-9\.$complement]*$/", $var)) $erreur = 1; break;
			case "int_list": if(! preg_match("/^[0-9\,$complement]*$/", $var)) $erreur = 1; break;
			case "string_list": if(! preg_match("/^[0-9a-zA-Z\,\._ÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿñ$complement]*$/", $var)) $erreur = 1; break;
			default: break;
		}		
